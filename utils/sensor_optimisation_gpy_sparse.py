...
def sensor_loc_optimisation_naive(k, sets, X, Y, model):
    ...
    for j in tqdm.tqdm_notebook(range(k), desc="Main Loop"):

        S_A = np.setdiff1d(S, A).astype(int)
        delta_y = np.zeros((n_S - j))
        ## Inner Loop : Iterating over the potential sensor places
        for i, y in tqdm.tqdm_notebook(enumerate(S_A), desc="Inner Loop"):
            A_ = np.setdiff1d(V, np.append(A, [y]))
            # Mutual Information Gain

            delta_y[i] = delta_mi(y, A, A_, X, Y, model)
...
def sensor_loc_optimisation_lazy(k, sets, X, Y, model):
    """ This function implements the Algorithm 1: Approximation algorithm
    for maximizing mutual information.
    ...
    # MAIN LOOP of the Algorithm : Iterating over the number of sensors to place
    for j in tqdm.tqdm_notebook(range(k), desc="Main Loop"):
        p_bar = tqdm.tqdm_notebook(range(n_S), desc="Inner Loop")
        ## INNER LOOP : Iterating over the potential sensor places
        while True:
            p_bar.update(1)
            ...
            if count == j:
                ...
                # Mutual Information Gain
                delta_y_opt = delta_mi(y_opt, A, A_, X, Y, model)
...
def delta_mi(i_predict, A, A_, X, Y, model):
    ...
    if A.shape[0] == 0:
        ...
        covA = model.kern.K(x_predict)

    else:
        model.set_XY(X=X[A, :], Y=Y[A, :])
        _, covA = model.predict_noiseless(x_predict)

    model.set_XY(X=X[A_, :], Y=Y[A_, :])
    _, covA_ = model.predict_noiseless(x_predict)
    return covA / covA_
