...
for ix_loop in range(n_random):
    results[ix_loop] = dict()
    ...
    ug=vtktools.vtu('../../data/small3DLSBU/LSBU_200.vtu')
    ...
    results[ix_loop]['y'] = y
    results[ix_loop]['xB'] = xB
    results[ix_loop]['xDA'] = xDA
    results[ix_loop]['errxDA'] = errxDA
    results[ix_loop]['MSExDA'] = MSExDA
    results[ix_loop]['errxB'] = errxB
    results[ix_loop]['MSExDA'] = MSExb

    results[ix_loop]['y_norm'] = LA.norm(y, 2)
    results[ix_loop]['xDA_norm'] = LA.norm(xDA, 2)
    results[ix_loop]['xB_norm'] = LA.norm(xB, 2)


results_df = pd.DataFrame(results)
results_df.to_pickle('results_da')
